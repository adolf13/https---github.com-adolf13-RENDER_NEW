...
# Всегда берём локальные модули RENDER_NEW. Родительская папка может содержать
# другой проект с одноимённым config.py (например, WEB_NST).
...
class DoorParams:
    ...
    # Фрезеровка. Для одной фрезы достаточно cutter_dxf_path; для нескольких
    # передаётся словарь {"номер слоя": "путь к DXF профиля"}.
    cutter_dxf_path: Optional[str] = None
    cutter_dxf_paths: Optional[Dict[str, str]] = None
    milling_depth_mm: float = 1.5
    milling_depths_mm: Optional[Dict[str, float]] = None
...
"""
    ...
    # Фрезеровка
    parser.add_argument('--cutter-dxf', dest='cutter_dxf_path',
                        help='DXF рабочего профиля фрезы')
