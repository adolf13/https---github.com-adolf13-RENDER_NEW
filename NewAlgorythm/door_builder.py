...
from geometry import make_door_front_plain, reconstruct_panel_rects, group_rects_into_panels, make_door_front_with_holes, make_raised_panel_meshes, make_metal_frame, make_wall, make_floor, bbox, rot, make_casing_bar, make_flat_quad
from milling import make_milling_sweep_meshes
from milling_boolean import subtract_sweeps_from_door
...
class DoorBuilder:
    ...
    def __init__(self, dxf_data: DXFData, params, inner: bool = False,
        ...
        # Объёмы снимаемого материала. Они не добавляются к двери как валики,
        # а используются как операнды boolean-вычитания.
        self.milling_sweep_meshes = {}
    ...
    def build(self) -> List[Tuple[o3d.geometry.TriangleMesh, str, Optional[str]]]:
        ...
        if self.dxf_data.milling_paths:
            profile_paths = (
                getattr(self.params, "cutter_dxf_paths", None)
                or getattr(self.params, "cutter_dxf_path", None)
            )
            if not profile_paths:
                cutter_root = os.path.join(os.path.dirname(os.path.abspath(__file__)), "cutters")
                automatic = {
                    cutter_id: os.path.join(cutter_root, f"{cutter_id}.dxf")
                    for cutter_id in self.dxf_data.milling_paths
                }
                missing = [path for path in automatic.values() if not os.path.isfile(path)]
                if missing:
                    raise ValueError(
                        "Для DXF с траекториями фрезеровки нужно передать "
                        "cutter_dxf_path/cutter_dxf_paths. Не найдены профили: "
                        + ", ".join(missing)
                    )
                profile_paths = automatic

            bounds = self.dxf_data.contour_bounds
            origin_mm = (bounds[0], bounds[1]) if bounds else (self.min_x, self.min_y)
            self.milling_sweep_meshes = make_milling_sweep_meshes(
                self.dxf_data.milling_paths,
                profile_paths,
                origin_mm=origin_mm,
                surface_z_m=self.door_thickness,
                scale=self.scale,
                top_extension_m=0.0002,
                milling_depth_mm=(
                    getattr(self.params, "milling_depths_mm", None)
                    or getattr(self.dxf_data, "milling_depths_mm", None)
                    or getattr(self.params, "milling_depth_mm", 1.5)
                ),
                milling_line_depths_mm=getattr(
                    self.dxf_data,
                    "milling_line_depths_mm",
                    None,
                ),
            )
            summary = ", ".join(
                f"{cutter_id}: {len(items)}"
                for cutter_id, items in self.milling_sweep_meshes.items()
            )
            print(f"🪚 Объёмы прохода фрез: {summary}")

        # 1–2. Корпус и лицевая сторона
        if panel_type == 'milling':
            sweep_meshes = [
                mesh
                for cutter_meshes in self.milling_sweep_meshes.values()
                for mesh in cutter_meshes
            ]
            carved_body = subtract_sweeps_from_door(
                self.door_width,
                self.door_height,
                self.door_thickness,
                sweep_meshes,
            )
            # Один цельный меш вместо корпуса и плоскости, которая раньше закрывала пазы.
            meshes.append((carved_body, "door_front", self.texture_path))
            print(f"✅ Фрезеровка вычтена из полотна: {len(sweep_meshes)} прохода")
        elif panel_type == 'groove':
            ...
            print("⚠️ Старые groove-валики отключены; нужны DXF-траектории и профиль фрезы")
        else:
            meshes.append((self._make_body(), "door_body", None))
        ...
        if self.params.frame_metal or self.inner:
            ...
            if frame_mesh:
                ...
                meshes.extend(self._make_casing(self.frame_texture_path))
        else:
